...
output_dir="${1:-$repo_root/web-dist/asymmetry-lab}"
...
cargo build -p asymmetry_lab --bin asymmetry_lab \
  --profile wasm-release --target wasm32-unknown-unknown

profile_dir="$target_dir/wasm32-unknown-unknown/wasm-release"
wasm="$(find "$profile_dir" -maxdepth 1 -name 'asymmetry?lab.wasm' -print -quit)"
if [[ -z "$wasm" ]]; then
  echo "cargo completed but no asymmetry_lab wasm was produced in $profile_dir" >&2
  exit 1
fi
...
"$bindgen" --target web --no-typescript \
  --out-dir "$output_dir" --out-name asymmetry_lab "$wasm"

# Content-address the bundle so a browser never serves a stale build from cache.
bundle="$output_dir/asymmetry_lab_bg.wasm"
build_hash="$(sha256sum "$bundle" | cut -c1-16)"
mv "$bundle" "$output_dir/asymmetry_lab_bg.$build_hash.wasm"

sed "s/__ASYMMETRY_BUILD__/$build_hash/g" \
  "$repo_root/labs/asymmetry_lab/web/index.html" > "$output_dir/index.html"

size="$(du -h "$output_dir/asymmetry_lab_bg.$build_hash.wasm" | cut -f1)"
echo "Asymmetry lab web bundle: $size"
