...
### Manual verification — the authored towers have been walked

**2026-08-07, by a human, on the shipped content.** Spectate AI walked the
facility and the authored perimeter towers are traversable. This is the first
time any of the vertical circulation work has been confirmed by a person rather
than by tests and headless bots, and it retires the largest standing risk on
that work: the 66 towers, the pierced floors, the thin flights and the ring
decks had all been verified only by instruments built alongside them.

Reported alongside it, and worth carrying into any polish pass: **the kit will
need hand-made tiles to really finish it.** Everything in the corpus today is
parametrically generated by `forge`. That observation is what moved the FGD's
version-3 gap from housekeeping to a blocker — see below.
