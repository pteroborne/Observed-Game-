...
- **New Special Room Types.** `[ ]` *(planned)* Add specialized room configurations to diversify gameplay depth and tactical coordination:
  - **"Master" Room:** A room that "breaks the rules" by having only a single mutating hallway connection. The remaining thresholds point directly to all other rooms in the facility, but they are strictly one-way (the exit threshold into the connecting room is invisible and non-traversable, preventing entry into the Master room through them).
  - **Tether Camera Room:** A presentation room containing visual wall monitors displaying each room currently hosting a placed player tether anchor.
  - **Guardian Observation Room:** Same as the tether room, but visual panels display live camera feeds representing the direct line of sight of each active guardian entity in the megastructure.
  - **Guardian Control Room:** An interactive console room that allows players to reassign active guardians to target other players. The reassignment remains active until the guardian succeeds or is reset by a tether.
- **Next:** playtest trap timing and route readability, then implement the new specialized room types in a dedicated gameplay-depth phase.
