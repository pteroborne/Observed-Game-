...
use observed_traversal::{
    ArenaSpec, FIXED_DT, FpsArena, FpsBody, FpsConfig, PhysicsBackend, TraversalWorld,
};
...
impl LockstepWorld {
    ...
    pub fn authored_with_backend(backend: PhysicsBackend) -> Self {
        ...
        let world = TraversalWorld::from_spec(ArenaSpec::from_legacy(&arena), backend, &config);
        Self {
            bodies: [
    ...
    pub fn step(&mut self, frame: LockstepFrame) {
        ...
        for (body, input) in self.bodies.iter_mut().zip(frame.inputs) {
            self.world
                .step_body(body, input.to_player_intent(), &self.config, FIXED_DT);
        }
        self.tick += 1;
    }
