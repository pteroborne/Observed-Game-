...
/// Walk all compiled orientations up and down, including both side lanes.
/// This catches portal/cheek intrusion and a flight that leaves its high sill
/// unsupported, neither of which a centre-only ascent screenshot can prove.
#[test]
fn processional_ascent_is_walkable_both_ways_in_all_rotations() {
    let source = crate::parse_authored_module(&crate::forge::silos::hall_ramp())
        .expect("ascent validates")
        .prototype;
    let config = FpsConfig::default();
    for turn in 0..6 {
        let mut tile = source.clone();
        ...
        for direction in [-1.0, 1.0] {
            for lane in [-1.5, 0.0, 1.5] {
                let start_x = 6.5 * direction;
                let finish_x = -start_x;
                let start = axis * start_x + side * lane;
                let finish = axis * finish_x + side * lane;
                let surface = |x: f32| 0.5 + (x + 7.0) * 8.0 / 14.0;
                let mut body = FpsBody::spawned(
                    Vec3::new(
                        start.x,
                        surface(start_x) + config.half_height + 0.05,
                        start.y,
                    ),
                    0.0,
                );
                assert!(
                    drive_capsule_to(&scene, &mut body, finish, &config),
                    "blocked ascent: turn {turn}, direction {direction}, lane {lane}"
                );
                let along = Vec2::new(body.position.x, body.position.z).dot(axis);
                let feet = body.position.y - config.half_height;
                assert!(
                    (feet - surface(along)).abs() < 0.2,
                    "left the flight: turn {turn}, direction {direction}, lane {lane}, height {feet}"
                );
            }
        }
    }
