...
fn protected_with_halo(
    ...
    // A protected corridor hall's walls, and a protected room's windows, follow its
    // lateral neighbours (`exposure::follows_neighbours`): one becoming built or
    // unbuilt opens or closes an edge of the hall, or a window of the room. So their
    // neighbours are held too, and the pocket shrinks around them rather than solving
    // a change that would have to be thrown away.
    let followers: Vec<HexCoord> = protected
        .iter()
        .copied()
        .filter(|coord| {
            world
                .placements
                .get(coord)
                .is_some_and(super::exposure::follows_neighbours)
        })
        .collect();
    for coord in followers {
