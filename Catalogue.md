...
- `kinetic_lab`: Continuous first-person kinetic playground in one authored industrial chamber. Crosshair-ray push/pull applies fixed-tick raw-Rapier impulses to minor Guardians and loose crates; the production character controller shares their collision world. Walls and props stagger or transfer momentum without damage, surviving lower geometry catches falls, and true void or a telegraphed retracting bridge provides environmental eliminations. Safe practice has labeled unlimited charge; a three-wave encounter adds pursuit, capture, finite charge, and generator-gated station recharge. The schematic and WFC siege are retired; `kinetic_fps` launches the same new experience. Simulation snapshots include durable physics state, renderer interpolation cannot change outcomes, and tests cover targeting, contact, falls, replay, scheduling, and repeated resets. Shared semantic styling, visible device labels, diagnostic collision/velocity overlays, and command-driven walkthrough evidence support the remaining human feel assessment.
- `wfc_kinetic_lab`: The kinetic tool inside a floor the production WFC solver actually built, rather than an authored chamber. Seven cells of a real 3x3 solve of the committed tile catalogue, projected to real authored hulls, with the two cells the collapse declined to build left as floor-less holes; the seed is a lab control and every value resolves to a playable floor. Reuses `kinetic_lab`'s raw-Rapier stepping and deterministic ray, `hex_wfc_lab`'s structural pass and per-register shell surface, and style-owned practical light budgets. Its finding: the tile corpus parapets every face onto void and never points a door at an unbuilt cell, so a solved floor offers no unrailed geometry to shove a minor off - the only drop is one the Architect makes, by retracting a tile so the doorways into it stop leading anywhere. Retraction removes one cell's mesh and colliders, drops what stood on it, and severs the crossing. Minors wear the shared `kinetic_lab::guardian` rig and the lab speaks with the shared `kinetic_lab::sound` design, both driven through a neutral per-frame interface rather than a second copy; retraction is heard at the tile that goes and a fall at the cell it went over. Tool force and minor speed are tunable while running and shown in the HUD; the starting force is above the authored chamber's because a 14 m cell is not a room you cross in a few strides. Sight, support and navigation query the broad phase against tagged structural colliders and share one route solve per tick, which is what keeps a full wave inside a frame. Generator-gated station recharge, three-wave encounters, snapshot/replay digest equality, and command-driven walkthrough evidence; the parapet finding and the frame budget are both kept as falsifiable tests.
