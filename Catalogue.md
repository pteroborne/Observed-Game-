...
- **`game/src/teleport/`:** Discrete room/hallway footprint building (`geom.rs`), navigation/connection tracking (`nav.rs`), crossing/transition math and spawning (`transition.rs`), and doorway gap geometry. The hallway library includes the two-level Gantry jump-map hall (height-gated thresholds, walkable `DeckSeg` decks, understory rerouting) proven in `observed_traversal::gantry` and `gantry_lab`.
- **`game/src/map_catalog.rs` & `map_validation.rs`:** Active map selection (`OBSERVED2_MAP`, defaulting to `sector_relay_v1`) with validated `MapSpec` builder plumbing, plus pure semantic-map geometry audits.
