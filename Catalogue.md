...
- **`game/src/hex_wfc/`:** Canonical Play adapter over `observed_match::hex_wfc`. `sim` owns fixed-step versioned commands, content-hashed strict/legacy authoring catalogues, replay/results, player-local discovery, nearby-solvable seed selection, and production 28×20×10 configuration; `view/` applies changed-cell geometry only and uses register-keyed `observed_style` materials, bounded streaming, fog/bloom, and a deterministic key-light budget; `lantern` projects the procedural caged anchor lantern and physical Guardian; the active-floor `tacmap` projects glimpsed/traversed/stale/anchored/current/exit/vertical knowledge without rival leakage. Opt-in gameplay, map, style, traversal, startup-lighting, and ten-mutation performance captures are inert in normal play.
...
- **`game/src/screens/`:** The state machine and screens. `screens.rs` is the menu domain (button/action types) plus the two composition plugins (`ScreensPlugin` for menu-like screens, `MatchPlugin` [DEPRECATED] for the first-person match). Submodules: `menu.rs`, `loadout.rs`, `lobby.rs`, `hud.rs`, `audio.rs`, `input.rs`, and `match_runtime/` [DEPRECATED] (the match's own lifecycle: `session.rs` enumerates the match-resource set once for setup/teardown and the no-leak test, `crossing.rs` owns immutable place snapshots plus exact forward/reverse threshold transactions, `ambience.rs` handles atmosphere/decohere feedback, `spectator.rs` drives the spectator bot, `input.rs` is match input), and `place/` [DEPRECATED] (the renderer: `factory.rs`/`shell.rs`/`monitors.rs`/`animate.rs`/`lighting.rs`/`mesh.rs`/`preview.rs`/`item_visuals.rs`, including isolated render-to-texture portal cameras, recomposed by a thin `mod.rs`).
