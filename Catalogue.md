...
4. **`observed_facility`** — [README](crates/observed_facility/README.md)
   - *Purpose:* Facility topology rules: authored room definition templates, transform alignments, port connectivity, overlapping geometry validation, static `MapSpec` WFC, and the experimental continuous multi-level `FullWfcWorld` with observation pinning plus weighted-A* relayout guards.
...
- **`game/src/full_wfc/`:** Selectable `GameState::FullWfc` experiment, launched from `Experiment: Full WFC` while legacy `Play` remains the teleport match. `sim.rs` owns the intent-driven continuous runner, observation frames, atomic five-second pulses, climb state, and movement-side exit reservation; `view.rs` selectively rebuilds only changed cells, renders style-owned neon-noir semantic signals, and drives the A*-cost candle without revealing the route; `input.rs` is the keyboard/mouse adapter. `OBSERVED2_CAPTURE_FULL_WFC=<path>` autostarts and captures this state.
