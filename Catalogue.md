...
- `tactics_lab`: Turn-based tactical variant of the canonical hex facility, built as a gameplay-tuning instrument for Arc T's shallow-choices and imperceptible-change findings. Its Guided preset reveals the complete map and exit on a single floor while leaving observation/freeze semantics intact; setup independently configures exact tiles per floor and one to ten floors. Its primary view is a single authored deck with ceilings removed and every perimeter hull capped at quarter height; its second view is a filled top-down operations map of the active deck with literal doorway gaps and compact up/down level arrows. Both views have independent pan/zoom poses beside a display-fixed, internally scrolling command dock with exclusive pointer ownership. Hover previews the exact AP-bounded route, movement resolves in visible beats, and every interaction and gear action has a control. A deterministic objective bot can run/pause or single-step the player squad through the ordinary logged action boundary for spectating and debugging. Setup, loading/error, pause/help and results form one player-facing shell. Ending the turn still runs the real observation-safe relayout, with the next pocket telegraphed so holding ground is a visible decision.
