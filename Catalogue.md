...
- `daydream_lab`: A single scene built with every authoring contract switched off - no seam, no port signature, no hull budget, no Legibility Contract. It exists to answer whether the saturated, mirror-pool architectural-painting look is worth wanting before anyone tries to make the tile system produce it. A colonnade standing in still water, a round arch onto a void that is fog rather than sky, and a real planar reflection: a second camera at the main camera's position mirrored through the waterline, sampled by the water shader at screen position. `OBSERVED2_DAYDREAM_HOUR` picks `noon` (ambient-led, shadowless, cold white void) or `late` (collapsed ambient, one warm raking sun, the piers striping the aisle); `OBSERVED2_DAYDREAM_VIEW` picks `axis`, `low`, or `aisle`.
