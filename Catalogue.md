...
18. **`observed_cutaway`** — [source](crates/observed_cutaway/src/lib.rs)
   - *Purpose:* Shared, colour-blind authored-hull projection for isometric cutaways: stable tile-key mesh caching, convex triangulation, ceiling/near-wall classification, focus/deck selection, and batched Bevy meshes. Extracted from Composition Studio when `tactics_lab` became its second consumer.
...
- `tactics_lab`: Turn-based tactical variant of the canonical hex facility, built as a gameplay-tuning instrument for Arc T's shallow-choices and imperceptible-change findings. Its primary view is a single authored, ceiling/near-wall-cut deck using the committed WFC hull catalogue; a stacked schematic remains the strategic overview. Hover previews the exact AP-bounded route, cursor shape states what a click will do, movement resolves in visible beats, and a viewport-inset command dock exposes every interaction and gear action. Setup, loading/error, pause/help and results form one player-facing shell. Ending the turn still runs the real observation-safe relayout, with the next pocket telegraphed so holding ground is a visible decision.
