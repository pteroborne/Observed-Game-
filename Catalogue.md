...
- **Tactics browser distribution:** `tactics_lab` has a validated embedded-catalog path whose cells, rooms, composition, and simulation hash match the filesystem loader. Its WebGL2 WASM shell uses one-finger tap/drag and two-finger pan/pinch, moves the fixed command dock below the physically inset map on portrait displays, and ships with repository build plus private-LAN gzip serving scripts.
