...
4. **`observed_facility`** — [README](crates/observed_facility/README.md)
   - *Purpose:* Facility topology rules: authored room definition templates, transform alignments, port connectivity, overlapping geometry validation, static `MapSpec` WFC, the demoted square `FullWfcWorld`, and the canonical authored-tile `HexWfcWorld` with blueprint/ramp/threshold pinning, bounded frontier mutation, protected halos, per-cell revisions, patch deltas, stable identity, and weighted-route guards.
...
6. **`observed_match`** — [README](crates/observed_match/README.md)
   - *Purpose:* The competitive match brain, containing:
     ...
     - `hex_wfc`: canonical hex-facility race state, strict cell/whole-room projection, incremental Rapier deltas, ramp/shaft movement, versioned commands, caged lantern anchors, the physical Guardian, player-local map knowledge, content-hashed replay, bounded mutation, and deterministic snapshots.
...
16. **`observed_authoring`** — [source](crates/observed_authoring/src/lib.rs)
   - *Purpose:* Pure TrenchBroom `.map` import and deterministic `tilec` pipeline: strict cell/whole-room schemas, footprint/port/floor/headroom/ramp/complexity contracts, SHA-256 hull catalogues, runtime rotation/register expansion, legacy-manifest compatibility, and brush-to-convex-collider projection.
...
- `hex_wfc_lab`: Arc L's retained 2D/3D proof surface: animated axial WFC collapse, level slices, blueprint/ramp/shaft validation, observation-safe relayout, exact authored-collider rendering, shared-controller walk/free-fly modes, and deterministic bot-POV/evidence captures.
...
- `iso_observer_lab`: Arc O's composition instrument and the arc's showcase surface. It solves a production-scale `HexWfcWorld`, projects its authored geometry, and draws the result as a retro-futuristic console schematic: line work on a dark screen with hue spent on one question — **green will not rewire, red the solver can**. The layer cycle (`Tab`: level 0 … N-1, then all) manages density rather than detail, so a single layer draws the **real authored hulls** (structural edges only; coplanar triangulation diagonals are filtered) while the whole stack falls back to cell shells. `D` restores the district-coloured solid massing as a dev view, and clicking a hex pins it and reports the resolved tile key, hull/vertex counts, archetype, ports, district, room role and whether the solver may move it. `[`/`]` cycle the five pinned seeds, `R` resolves, `Esc` clears. `OBSERVED2_CAPTURE=<dir>` walks every seed and writes a stacked overview, per-level slices, an inspector frame and a census `manifest.json`. Unlike the in-game map it renders **ground truth**, because its job is to audit the solver rather than to be a survivor's sketch.
...
- **`server/src/lib.rs`:** Bevy-free 60 Hz session host shared by dedicated and listen-server modes. It owns the four-seat 2v2 lobby, bot fill/takeover, reconnect reservations, content/input compatibility, canonical hex simulation, retained authoritative command/digest history, late-join synchronization, and one-shot desync replay.
...
- **`game/src/main.rs` & `lib.rs`:** `main.rs` is a one-line binary entrypoint; `lib.rs` owns Bevy app/plugin composition, the `GameState` state machine (Splash → Main Menu → Loadout → Lobby → `HexWfc` → Results, with `Match` and `FullWfc` regression states), and the top-level camera/light setup. `arch_check.rs` is a `#[cfg(test)]`-only ratchet: source-scanning tests fail if a glob re-export, a non-test `use super::*`, a simulation-to-presentation import, or an oversized canonical WFC file creeps back in.
- **`game/src/hex_wfc/`:** Canonical Play adapter over `observed_match::hex_wfc`. `sim` owns fixed-step versioned commands, content-hashed authoring catalogues, replay/results, two-member team knowledge, nearby-solvable seed selection, local play, and authoritative LAN-frame replay/desync recovery; `view/` applies changed-cell geometry only and uses register-keyed `observed_style` materials, bounded streaming, fog/bloom, and a deterministic key-light budget; `lantern` projects the procedural caged anchor lantern and physical Guardian; `view/map/` is the Tab-toggled full-screen isometric survivor map (Arc O Phase 105, promoted from `iso_observer_lab`), drawing shared teammate knowledge only, on its own render layer with its own key. It shows what the tiles *compose* on five non-competing channels: colour is the district, height is the archetype, footprint width separates room/hallway/vertical (rooms meet with no seam so a multi-cell room reads as one space), link bars mark every port pair seen open from both sides, and a cap marks a cell the team is holding. You/exit cells are recoloured to signal tier. Stability is derived only from leak-free inputs — never the global observation frame, which would surface rival positions.
- **`game/src/full_wfc/`:** Demoted Arc-K square regression adapter, reachable from Play only through `OBSERVED2_MAP=square` (or direct tests). Its versioned simulation, shell, audiovisual projection, and regression suites stay intact; new production features target `hex_wfc` instead.
- **`game/src/sim/`:** Simulation-side Bevy resources — no rendering, UI, or asset types. `director.rs` holds `MatchDirector`, the single owner of the live networked match plus the elimination series (`tick`, `run_to_completion`, `outcome`, spectator pumping, and forcing/suppressing scripted rounds for evidence capture). `state.rs` holds the teleport/body/intent/spectator/lobby resources (`TeleportState` [DEPRECATED], `SpectatorBot`, `MatchIntent` [DEPRECATED], `ItemIntent` [DEPRECATED], `MatchPaused` [DEPRECATED], `LobbyRuntime`, etc.). `nav.rs` is the pure brain→`Nav` projection used for bot pathing.
...
- **`game/src/lan.rs` & `screens/lan.rs`:** Persistent LAN client/listen-host ownership plus broadcast discovery, direct-IP entry, connection transitions, and authoritative roster/countdown projection. `screens/lobby.rs` offers readiness and team requests; server state remains the source of truth.
- **`game/src/screens/`:** The state machine and screens. `screens.rs` is the menu domain plus screen composition; `menu.rs`, `lan.rs`, `lobby.rs`, `loadout.rs`, settings, replay, and the deprecated isolated-Place regression screens stay state-scoped and clean up on exit.
