...
4. **`observed_facility`** — [README](crates/observed_facility/README.md)
   - *Purpose:* Facility topology rules: authored room definition templates, transform alignments, port connectivity, overlapping geometry validation, static `MapSpec` WFC, the demoted square `FullWfcWorld`, and the canonical authored-tile `HexWfcWorld` with blueprint/ramp/threshold pinning, incremental relayout work, stable identity, and weighted-route guards.
...
6. **`observed_match`** — [README](crates/observed_match/README.md)
   - *Purpose:* The competitive match brain, containing:
     ...
     - `hex_wfc`: canonical hex-facility race state, authored geometry projection, ramp/shaft movement, objective bots, observation-safe relayout, and versioned deterministic snapshots.
...
16. **`observed_authoring`** — [source](crates/observed_authoring/src/lib.rs)
    - *Purpose:* Pure TrenchBroom `.map` import, authored tile schemas/manifests, exact-footprint validation, and brush-to-convex-collider projection for the canonical hex tile library.
...
- **`game/src/hex_wfc/`:** Canonical Arc-L Play adapter over `observed_match::hex_wfc`. `sim` owns the fixed-step match, abstract `PlayerIntent`, replay/result integration, local survivor knowledge, nearby-solvable seed selection, and production 28×20×10 configuration; `view/` instances authored collider geometry with register-keyed `observed_style` materials, bounded streaming, fog/bloom, and a deterministic key/headlamp budget; `entities`, `hud`, `tacmap`, `feedback`, `cues`, and `audio` project stable actors, the exit, true axial hex outlines, relayout warnings/commits, and semantic one-shots. Opt-in gameplay/map/style/relayout/traversal captures live with the adapter and are inert in normal play.
