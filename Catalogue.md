...
- `iso_observer_lab`: Arc O's composition instrument. It solves a production-scale `HexWfcWorld` and renders it in an orthographic isometric projection on two orthogonal channels — colour is the architecture register (via `observed_style::architecture_surface`, never invented locally), height is the hex archetype — so district contiguity, per-district composition, and vertical clustering are all readable in one frame. Exact quantized-hexagon prisms, stacked/slice views, a live archetype and district-region census, `[`/`]` seed cycling, and `R` reset. `OBSERVED2_CAPTURE=<dir>` walks all five pinned baseline seeds, writes a stacked overview plus per-level slices and a census `manifest.json`, then exits. The Phase 105 in-game map is promoted from this renderer, but reads player knowledge instead of ground truth.
