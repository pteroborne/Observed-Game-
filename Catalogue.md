...
- `iso_observer_lab`: Arc O's composition instrument, and the showcase surface for every phase in the arc. It solves a production-scale `HexWfcWorld` and renders it in an orthographic isometric projection, drawing what the tiles *compose* rather than where they sit: colour is the district accent, height is the archetype and width is the composition (both from `observed_style::hex_sketch`, the same table the in-game map reads, so the two cannot drift), rooms fill their hex and merge into one mass, hallways are ribbons, and bars mark every open port pair with risers joining floors. Exact quantized-hexagon prisms, stacked/slice views, a live archetype + composition + connection + district-region census, `[`/`]` seed cycling, and `R` reset. `OBSERVED2_CAPTURE=<dir>` walks all five pinned baseline seeds and writes a stacked overview, per-level slices and a census `manifest.json`. Unlike the in-game map it renders **ground truth**, because its job is to audit the solver rather than to be a survivor's sketch.
