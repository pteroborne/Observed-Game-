...
- **`game/src/main.rs` & `lib.rs`:** `main.rs` is a one-line binary entrypoint; `lib.rs` owns Bevy app/plugin composition, the `GameState` state machine (Splash → Main Menu → Loadout → Lobby → Match [DEPRECATED] → Results), and the top-level camera/light setup. `arch_check.rs` is a `#[cfg(test)]`-only ratchet: source-scanning tests that fail the build if a glob re-export, a non-test `use super::*`, or a `sim/` → `view`/`screens` import creeps back in.
- **`game/src/full_wfc/`:** Canonical Arc-K match adapter over `observed_match::full_wfc`: `sim` owns versioned input frames and results/replay integration; `view/` consumes the shared render/Rapier geometry snapshot and separates streamed shell/threshold projection, per-register procedural dressing, style-normalized environment assets, and the bounded desktop lighting/post-process rig (HDR bloom, high SSAO, FXAA, fog, one shadow key, and local practical budget) with smooth interpolation to prevent jarring threshold-crossing transitions; `entities` projects eight actors, Guardian, objectives, and stable equipment; `tacmap` draws team-local traversed/glimpsed/stale/anchored knowledge; `audio`, `cues`, and `feedback` exhaustively cover gameplay events and proximity/mutation pressure.
