...
- `tactics_lab`: Turn-based, whole-board tactical variant of the canonical hex facility, built as a gameplay-tuning instrument for Arc T's shallow-choices and imperceptible-change findings. A squad spends action points moving cell to cell; ending the turn runs the real observation-safe relayout. The pocket that will re-collapse is **telegraphed** at the start of the turn, so holding ground against the facility becomes a visible decision rather than an invisible one. Isometric and top-down views share one geometry builder and one cell-classification pass. Every rule is a **match setting** on a player-facing setup screen (`observed_ui` widgets), and every command has a pointer control as well as a key. Reuses `HexWfcWorld`, `HexPlayerMapKnowledge`, `HexLanternState`, `HexPadState` and `HexGuardianStatus` directly rather than modelling them; drops bodies, physics, and sub-room objective placement.
