...
- `architect_lab`: First playable Architect Ascent proof from the Rogue Architect's ground-truth schematic. Three deterministic small modes cover a 6x5x1 Pocket Pursuit, 8x6x2 Quick Climb, and 10x8x2 Full Ascent; each uses the same real `HexWfcWorld` and authoritative five-card tile/door command shape. Observation, occupancy, prison-core protection, district matching, rotation, local attachment, and the shared 300-tick cooldown are simulation-owned legality. Autonomous eye Observers and pyramidal Guardians choose intents through traced fixed-tick behavior trees, while an optional Rogue Architect tree uses the exact human command path for unattended runs. Its playful map-first command surface presents ascending floors, a scenario carousel, mutation-charge meter, and fanned clickable cards whose code-drawn miniature hex art combines district-specific ribs or grid cells, a distinct deployable-door silhouette, and readable branch topology. Target legality, live topology previews, and cursor-anchored pan/zoom remain visible. Capture sends Observers to the protected prison core and all-jailed resolves the Rogue outcome. Contradictions are identified on the live topology; staged retraction is the next lab increment.
- `kinetic_lab`: The Observer's half of the Architect Ascent step-B proof, asking one question — is a shove that commits a minor Guardian to void deterministic, readable, and fair? A pure fixed-tick `KineticWorld` over `observed_hex` coordinates owns every rule; `lab.rs` renders and reads keys but decides nothing. The authored single floor puts one feature behind each rule: a void rim to shove over, an unrailed ledge run that carries momentum past its own impulse, a retracting tile that kills on a delay rather than on contact, a wall that blocks a shove outright, a recharge station, and the generator that powers it. The tool deals no damage and cannot touch a rival — architecture is the only thing that kills — while charge is finite and restored only at a live station, making the generator/station/tool dependency visible in one board. One major Guardian freezes under observation and two minors deliberately do not, so the asymmetry is demonstrated rather than asserted. Because `resolve_shove` is pure, the lab previews the exact destination and fate before the trigger is pulled, with a second ring on lethal outcomes so nothing rides on hue alone; preview and outcome are the same function, so any disagreement is a model bug. Shove travel is a discrete walk along hex faces with a travel cap, never a physics query, and a 600-tick scripted digest comparison holds the determinism contract. Coverage includes every shove fate, both directions of the observation asymmetry, power gating on recharge and sight, refusals, preview purity, one-keypress-one-shove through the real Bevy schedule, and ten resets with no entity leak. Two binaries present the one model: `kinetic_lab` is the top-down schematic, and `kinetic_fps` is first person over the identical `KineticWorld`. Embodiment keeps the rule boundary intact — `model.rs` owns every rule on whole cells, `embodied.rs` owns the continuous body and reports back exactly two derived facts per tick (the cell stood on, the face looked down), so the shove still resolves on the lattice in fixed-tick simulation while the player walks and aims freely; `observed_traversal::step_body` is already pure at `FIXED_DT`, which is this model's tick, and a 600-tick scripted run holds both the board digest and the body equal. The lattice tiles exactly with 14x12 floor plates sheared 7 per row, so plates meet without gap or overlap and void leaves an exact hole to fall through; the hex lattice remains connectivity and targeting rather than plate geometry, and rendering matches collision. Rank reads as the order of the solid — cube minors, a tetrahedral major — and both snap between cells and hold, the determinism constraint turned into the clockwork read, with a compile-time assert keeping the snap inside the step interval. The human gate is unchanged and open: whether a shove is satisfying is decided at the keyboard.
...
- `daydream_lab`: Scenes built with every authoring contract switched off - no seam, no port signature, no hull budget, no Legibility Contract. It exists to answer whether a look is worth wanting before anyone tries to make the tile system produce it. The lab is split into a rig (camera, planar reflection, gradient sky, fog, screen-space occlusion) and interchangeable rooms that hand it a `Staging`. `OBSERVED2_DAYDREAM_SCENE` picks `hall` (a colonnade standing in still water, two arched walls one seen through the other, steps climbing out of the pool) `screen` (the Shadow Screen: three tatami rooms deep, separated by cross-walls with one opening each, walled in backlit paper that is itself the light source; the kumiko grid is drawn into the emissive texture while the stiles and rails that cast are geometry, and the sun arrives through them already striped), `backrooms` (the Overlit Grid: one yellow at three values, a suspended ceiling at head height that is also the only light source, an irregular scatter of wall stubs and no axis at all, and deliberately no reflection - a mirror would make it beautiful, which is the wrong answer), `monument` (the Facet Monument: eleven cells of held axis under tapered jade A-frame ribs, chevron walkways cantilevered over a polished floor, light only ever in the reveal between two masses, terminating on a raised dais with nothing on it), or `shaft` (a hexagonal wellshaft twelve levels deep and two cells across, built on the catalogue's own 8 m level and 7 m apothem, wrapped by a great helical stair that climbs one level per turn, its wall lit by emissive windows and standing lamps, and its darkness painted into per-level materials rather than lit for). `OBSERVED2_DAYDREAM_HOUR` picks `noon` or `late`; `OBSERVED2_DAYDREAM_VIEW` picks a standpoint the scene defines. The water is a real planar reflection - a second camera at the main camera's position mirrored through `y = 0`, sampled by the water shader at screen position.
