...
- **`game/src/full_wfc/`:** Canonical Arc-K match adapter over `observed_match::full_wfc`: `sim` owns versioned input frames and results/replay integration; `view/` consumes the shared render/Rapier geometry snapshot and separates streamed shell/threshold projection, per-register procedural dressing, style-normalized environment assets, and the bounded desktop lighting/post-process rig (HDR bloom, high SSAO, FXAA, fog, one shadow key, and local practical budget); `entities` projects eight actors, Guardian, objectives, and stable equipment; `tacmap` draws team-local traversed/glimpsed/stale/anchored knowledge; `audio`, `cues`, and `feedback` exhaustively cover gameplay events and proximity/mutation pressure. The legacy teleport-place match remains only as a regression fixture. `OBSERVED2_CAPTURE_FULL_WFC=<path>` captures gameplay, `OBSERVED2_CAPTURE_FULL_WFC_MAP=<path>` captures the survivor sketch, and `OBSERVED2_CAPTURE_FULL_WFC_STYLE=<dir>` captures all nine architecture registers.
...
- **`game/src/screens/`:** The state machine and screens. `screens.rs` is the menu domain (button/action types) plus the two composition plugins (`ScreensPlugin` for menu-like screens, `MatchPlugin` for the first-person match). Submodules: `menu.rs`, `loadout.rs`, `lobby.rs`, `hud.rs`, `audio.rs`, `input.rs`, and `match_runtime/` (the match's own lifecycle: `session.rs` enumerates the match-resource set once for setup/teardown and the no-leak test, `crossing.rs` owns immutable place snapshots plus exact forward/reverse threshold transactions, `ambience.rs` handles atmosphere/decohere feedback, `spectator.rs` drives the spectator bot, `input.rs` is match input), and `place/` (the renderer: `factory.rs`/`shell.rs`/`monitors.rs`/`animate.rs`/`lighting.rs`/`mesh.rs`/`preview.rs`/`item_visuals.rs`, including isolated render-to-texture portal cameras, recomposed by a thin `mod.rs`).
