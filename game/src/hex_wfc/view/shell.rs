...
use observed_match::hex_wfc::{
    HexLightSource, HexStructurePiece, HexStructureRole, HexTrimPiece, derive_thresholds,
    derive_trim_for,
};
...
/// The gate model's own width, so it can be scaled to an aperture.
...
pub(super) fn spawn_thresholds(
    ...
    // building.
    let scale = Vec3::splat(gate.scale * THRESHOLD_APERTURE / GATE_NATIVE_WIDTH);
